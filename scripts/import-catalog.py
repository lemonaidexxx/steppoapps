...
for index, row in enumerate(sheet.iter_rows(min_row=3, values_only=True), 3):
    ...
    rows.append(dict(id=stable('off_', list(row[:6])), courseId=stable('crs_', [name]), courseName=name, modality='Not specified' if mode=='Not Specified' else mode, hours=hours, institution=center, city=city, selectable=True, sourceRow=index, sourceStatus=status or ''))
